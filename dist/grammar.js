...
(function () {
    ...
    var grammar = {
        ...
        ParserRules: [
            ...
            { "name": "line", "symbols": ["line$subexpression$1"], "postprocess": (data) => {
                    return data[0][0];
                }
            },
            ...
            { "name": "bind", "symbols": ["variable", "bind$ebnf$1", (lexer.has("eq") ? { type: "eq" } : eq), "bind$ebnf$2", "expression"], "postprocess": (data) => {
                    return {
                        type: 'bind',
                        name: data[0],
                        value: data[4]
            ...
            { "name": "expression", "symbols": [(lexer.has("lp") ? { type: "lp" } : lp), "expression", (lexer.has("rp") ? { type: "rp" } : rp)], "postprocess": (data) => {
                    return data[1];
                }
            },
            { "name": "abstraction", "symbols": [(lexer.has("lambda") ? { type: "lambda" } : lambda), "variable", (lexer.has("dot") ? { type: "dot" } : dot), "expression"], "postprocess": (data) => {
                    return {
                        type: 'abstraction',
                        variable: data[1],
                        definition: data[3]
            ...
            { "name": "application", "symbols": [(lexer.has("lp") ? { type: "lp" } : lp), "expression", (lexer.has("ws") ? { type: "ws" } : ws), "expression", (lexer.has("rp") ? { type: "rp" } : rp)], "postprocess": (data) => {
                    return {
                        type: 'application',
                        function: data[1],
                        argument: data[3]
            ...
            { "name": "variable", "symbols": [(lexer.has("v") ? { type: "v" } : v)], "postprocess": (data) => {
                    return {
                        type: 'variable',
                        name: data[0].value
                    };
