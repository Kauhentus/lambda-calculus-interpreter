...
const result : any[] = parser.results[0].lines;
console.log("Valid programs:", parser.results.length)

// logging functions
const jsn = (o : any) => JSON.stringify(o);
const js = (o : any) => JSON.stringify(o, null, 4);
...
const rename = (obj : any, id : string) => {
    for(let key of Object.keys(obj)){
        if(typeof obj[key] === "object") obj[key] = rename(obj[key], id);
        else if(key === "name" || key === "variable") obj[key] = obj[key] + '_' + id;
    }
    return obj;
}

const dename = (obj : any) => {
    for(let key of Object.keys(obj)){
        if(typeof obj[key] === "object") obj[key] = dename(obj[key]);
        else if(key === "name" || key === "variable") obj[key] = obj[key].split("_")[0]
    }
    return obj;
}
...
const substitute = (expression : any, oldTerm: any, replacingTerm : any) => {
    ...
    if(expression.type === "abstraction"){
        ...
        else expression.definition = substitute(expression.definition, oldTerm, replacingTerm);
    }
...
const betaReduce = (term : any) : any => {
    ...
    if(term.type !== 'application') return term;
